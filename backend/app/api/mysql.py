...
class MySQLSchemaInfo(BaseModel):
    name: str
...
def _resolve_database(secret: Dict[str, Any], schema: str) -> str:
    """A MySQL schema is a database, so the requested schema wins over the credential default."""
    database = schema.strip() or str(secret.get("database") or "").strip()
    if not database:
        raise ValueError("The selected credential does not define a database.")
    return database


def _read_schemas(secret: Dict[str, Any]) -> List[MySQLSchemaInfo]:
    ...
    names = [row["name"] for row in rows if row["name"] not in _SYSTEM_SCHEMAS]
    default_database = str(secret.get("database") or "").strip()
    if default_database and default_database not in names:
        names.insert(0, default_database)
    return [MySQLSchemaInfo(name=name) for name in names]


def _read_tables(secret: Dict[str, Any], search: str, schema: str) -> List[MySQLTableInfo]:
    database = _resolve_database(secret, schema)
    pattern = f"%{search}%"
    with mysql_connection(secret) as connection:
        with connection.cursor() as cursor:
            cursor.execute(
...
def _read_columns(secret: Dict[str, Any], table: str, schema: str) -> List[MySQLColumnInfo]:
    database = _resolve_database(secret, schema)
    with mysql_connection(secret) as connection:
        with connection.cursor() as cursor:
            cursor.execute(
...
@mysql_router.get("/schemas", response_model=List[MySQLSchemaInfo])
async def list_mysql_schemas(
    credential_id: uuid.UUID,
    ...
    try:
        return await asyncio.to_thread(_read_schemas, secret)
    except Exception as exc:
        logger.warning("MySQL schema discovery failed for credential %s: %s", credential_id, exc)
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Could not load MySQL schemas") from exc
...
async def list_mysql_tables(
    ...
    schema: str = Query("", max_length=64),
    current_user: User = Depends(get_current_user),
    db: AsyncSession = Depends(get_db_session),
    credential_service: CredentialService = Depends(get_credential_service_dep),
):
    secret = await _mysql_secret(credential_id, current_user, db, credential_service)
    try:
        return await asyncio.to_thread(_read_tables, secret, search.strip(), schema.strip())
    except Exception as exc:
...
async def list_mysql_columns(
    ...
    secret = await _mysql_secret(credential_id, current_user, db, credential_service)
    try:
        return await asyncio.to_thread(_read_columns, secret, table.strip(), schema.strip())
    except Exception as exc:
