...
class CryptographyNode(ProcessorNode):
    ...
    def __init__(self):
        ...
        self._metadata = {
            ...
            "properties": [
                ...
                NodeProperty(
                    name="pbkdf2_salt",
                    displayName="PBKDF2 Salt",
                    type=NodePropertyType.TEXT,
                    description="Optional salt override for AES-256-GCM key derivation. Leave empty to use the default.",
                    required=False,
                    default="",
                ...
                NodeProperty(
                    ...
                    description="Optional iteration override for AES-256-GCM key derivation. Leave empty to use the default.",
                    required=False,
                    default=None,
                    min=1,
                    max=1000000,
    ...
    def _derive_env_compatible_aes256_key(
        self,
        passphrase: str,
        ...
        resolved_salt = salt if salt not in (None, "") else "randomSalt"
        salt_bytes = resolved_salt if isinstance(resolved_salt, bytes) else str(resolved_salt).encode("utf-8")

        try:
            resolved_iterations = int(iterations) if iterations not in (None, "") else 65536
    ...
    def execute(self, inputs: Dict[str, Any], connected_nodes: Dict[str, Any]) -> Dict[str, Any]:
        ...
        pbkdf2_salt = inputs.get("pbkdf2_salt") or getattr(self, "user_data", {}).get("pbkdf2_salt", "")
        pbkdf2_iterations = inputs.get("pbkdf2_iterations") or getattr(self, "user_data", {}).get("pbkdf2_iterations", None)
        ...
        try:
                ...
                if crypto_type == "symmetric":
                    if cipher == "base64":
                        ...
                        if cipher == "aes-256-gcm":
                            derived = self._derive_env_compatible_aes256_key(key, pbkdf2_salt, pbkdf2_iterations)
            ...
            return {
                "output": "",
                "success": False,
                "error": f"Cryptographic operation failed: {error_text}"
            }
