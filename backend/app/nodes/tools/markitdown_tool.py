"""
KAI-Flow MarkItDown Tool - Enterprise MinIO-backed Document Conversion
=======================================================================

This node provides a production-ready document conversion tool that integrates
MarkItDown library with MinIO object storage. It enables AI agents to convert
various document formats (PDF, DOCX, PPTX, XLSX, images, audio, video) to
Markdown format for processing and analysis.

Key Features:
- MinIO/S3-compatible storage integration with credential management
- Support for 20+ document formats via MarkItDown
- Optional OpenAI GPT-4o integration for enhanced OCR and audio transcription
- Configurable file size limits and security controls
- Comprehensive error handling and logging
- Temporary file cleanup and resource management

Architecture:
- Node Type: ProviderNode (creates LangChain Tool for agents)
- Storage Backend: MinIO/S3 with encrypted credential storage
- Conversion Engine: MarkItDown library with optional OpenAI enhancement
- Output Format: Markdown with source attribution

Supported Document Formats:
- Documents: PDF, DOCX, PPTX, XLSX, CSV, JSON, XML, HTML
- Images: PNG, JPG, GIF, BMP (with OCR if OpenAI configured)
- Media: Audio/Video transcription (requires OpenAI)
- Code: Various programming languages with syntax preservation
- Archives: ZIP extraction and content conversion

Usage Pattern:
1. Configure MinIO credentials in KAI-Flow credential manager
2. Add MarkItDown Tool node to workflow
3. Connect to agent node (ReactAgent, etc.)
4. Agent can invoke tool to convert documents on-demand
5. Converted markdown content returned for further processing
...
Version: 2.1.0 - MinIO Integration + OpenRouter Support
