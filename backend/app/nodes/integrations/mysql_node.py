...
class MySQLNode(ProcessorNode):
        ...
        self._metadata = {
            ...
            "properties": [
                ...
                NodeProperty(
                    ...
                    description="Database that holds the table. Defaults to the credential's database.",
                    placeholder="kai_demo",
                    default="",
    ...
    def execute(self, inputs: Dict[str, Any], connected_nodes: Dict[str, Any] | None = None) -> Dict[str, Any]:
        ...
        # Everything the node reports lives under a single "output" payload so the
        # JSON viewer does not repeat the same rows under a second heading.
        output: Dict[str, Any] = {
            "success": not errors,
            "operation": operation,
            "rows": rows,
    ...
    def _table_identifier(cls, inputs: Mapping[str, Any]) -> str:
        """Qualify the table with the selected schema unless it already carries a database."""
        table = str(inputs.get("table") or "").strip()
        schema = str(inputs.get("schema") or "").strip()
