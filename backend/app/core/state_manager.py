...
>>>>>>> serialization_fixes
    ...
    def _start_cleanup_thread(self):
        ...
        self._cleanup_thread = threading.Thread(
            target=self._cleanup_worker, name="StateManager-Cleanup", daemon=True
        )
    ...
    def _force_cleanup(self):
        ...
        with self._lock:
            ...
            sorted_sessions = sorted(
                self._state_metrics.items(), key=lambda x: x[1].last_accessed
            )
        ...
        try:
            ...
            if memory_percent > self.cleanup_policy.force_cleanup_threshold:
                ...
                if memory_percent > 0.9:  # 90% memory usage
                    logger.error(
<<<<<<< HEAD
                        f" Critical memory usage: {memory_percent:.1%}, forcing cleanup"
=======
                        f"🚨 Critical memory usage: {memory_percent:.1%}, forcing cleanup"
>>>>>>> serialization_fixes
                    )
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        with self._lock:
            total_memory = sum(
                metrics.memory_size_bytes for metrics in self._state_metrics.values()
            )

            total_nodes = sum(
                metrics.node_count for metrics in self._state_metrics.values()
            )

            total_variables = sum(
                metrics.variable_count for metrics in self._state_metrics.values()
            )
    ...
    def optimize_memory(self):
        ...
        with self._lock:
            ...
            logger.info(
                f" Memory optimization complete: {stats['total_memory_mb']:.1f}MB across {stats['active_sessions']} sessions"
            )
