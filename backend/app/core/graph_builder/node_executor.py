...
class NodeExecutor:
    ...
    def setup_node_session(
        self, gnode: GraphNodeInstance, state: FlowState, node_id: str
    ) -> None:
        """
        ...
        try:
            ...
            if gnode.type in PROCESSOR_NODE_TYPES and hasattr(
                gnode.node_instance, "session_id"
            ):
        ...
        try:
            ...
            if is_terminator:
                ...
                if inspect.iscoroutinefunction(execute_method):
                    result = self._execute_async_method_terminator(
                        execute_method, previous_node, user_inputs, node_id
                    )
                else:
                    result = execute_method(
                        previous_node=previous_node, inputs=user_inputs
                    )
            else:
                # PROCESSOR nodes expect (inputs, connected_nodes) signature
                if inspect.iscoroutinefunction(execute_method):
                    # Handle async execute method
                    result = self._execute_async_method(
                        execute_method, user_inputs, connected_nodes, node_id
                    )
                else:
                    # Handle sync execute method
                    # Use keyword arguments for better compatibility with different node signatures
                    result = execute_method(
                        inputs=user_inputs, connected_nodes=connected_nodes
                    )
            ...
            if gnode.type in PROCESSOR_NODE_TYPES:
                serializable_result = make_json_serializable_with_langchain(
                    processed_result, filter_complex=True
                )
                serializable_output = last_output
                logger.debug(
                    f"Agent serializable output: {type(serializable_output)} - '{str(serializable_output)[:100]}...'"
                )
            else:
                serializable_result = make_json_serializable_with_langchain(
                    processed_result, filter_complex=False
                )
            ...
            logger.debug(
                f"Output: '{last_output[:80]}...' ({len(str(last_output))} chars)"
            )
            ...
            raise NodeExecutionError(
                ...
                input_connections=getattr(
                    gnode.node_instance, "_input_connections", {}
                ),
                output_connections=getattr(
                    gnode.node_instance, "_output_connections", {}
                ),
            ) from e

    def execute_standard_node(
        self, gnode: GraphNodeInstance, state: FlowState, node_id: str
    ) -> Dict[str, Any]:
        """
        ...
        try:
            logger.info(
                f"[PROCESSING] Executing standard node: {node_id} ({gnode.type})"
            )
            ...
            try:
                ...
                logger.warning(
                    f"[TEMPLATE] Failed to store standard node output for {node_id}: {e}"
                )
    ...
    def extract_user_inputs_for_processor(
        self, gnode: GraphNodeInstance, state: FlowState
    ) -> Dict[str, Any]:
        """
        ...
        for input_spec in input_specs:
            if not input_spec.is_connection:
                ...
                if isinstance(inputs_group, dict) and input_spec.name in inputs_group:
                    ...
                    logger.debug(
                        f"Found user input {input_spec.name} in user_data['inputs']"
                    )
                # 2) Then: data[...] (flat user_data field)
                    ...
                    logger.debug(
                        f"Found user input {input_spec.name} in user_data (top-level)"
                    )
                # 3) Then: state.variables
                    ...
                    logger.debug(
                        f"Found user input {input_spec.name} in state.variables"
                    )
                # 4) Default value
                elif input_spec.default is not None:
                    user_inputs[input_spec.name] = input_spec.default
                    logger.debug(
                        f"Using default for {input_spec.name}: {input_spec.default}"
                    )
                # 5) Skip if not required and no value found
                    ...
                    logger.debug(
                        f"No value found for required input {input_spec.name}, using default if any"
                    )
    ...
    def _render_template_string(
        self, template_str: str, context: Dict[str, Any], node_id: str
    ) -> str:
        """
    ...
    def _apply_node_output_templating(
        self, gnode: GraphNodeInstance, user_inputs: Dict[str, Any], state: FlowState
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            for other_node_id in state.node_outputs.keys():
                ...
                try:
                    node_meta_model = getattr(
                        graph_node.node_instance, "metadata", None
                    )
                except Exception:
                    node_meta_model = None
    ...
    def extract_connected_node_instances(
        self, gnode: GraphNodeInstance, state: FlowState
    ) -> Dict[str, Any]:
        """
        ...
        if has_pool:
            logger.debug(
                f"[CONNECT] Extracting pool-enabled connections for {gnode.id}"
            )
        else:
            logger.debug(
                f"[CONNECT] Extracting connections for {gnode.id} (Clean Architecture)"
            )
        ...
        try:
            ...
            if has_pool:
                logger.debug(
                    f"[SUCCESS] Pool-aware connection extraction completed: {len(connected)} connections"
                )
            else:
                logger.debug(
                    f"[SUCCESS] Clean connection extraction completed: {len(connected)} connections"
                )

            # CRITICAL FIX: Validate connection formats before returning
            validated_connected = self._validate_and_fix_connection_formats(
                connected, gnode.id
            )
    ...
    def _extract_connected_node_instances_legacy_safe(
        self, gnode: GraphNodeInstance, state: FlowState
    ) -> Dict[str, Any]:
        """
        ...
        for (
            ...
            try:
                ...
                if isinstance(connection_data, list):
                    logger.debug(
                        f"[LEGACY] Processing {len(connection_data)} multiple connections for {input_name}"
                    )
                    aggregated_data = self._process_multiple_connections(
                ...
                # Handle single connection (dict format) - backward compatibility
                    ...
                    logger.debug(
                        f"[LEGACY] Processing single connection for {input_name}"
                    )
                    single_data = self._process_single_connection(
                        connection_data, state
                    )
                    ...
                    logger.error(
                        f"[ERROR] Failed to extract connection {input_name}: {type(connection_data)} is not a supported format"
                    )
                    logger.error(
                        f"[ERROR] Expected dict or list, got: {connection_data}"
                    )
                    connected[input_name] = None

            except Exception as e:
                logger.error(
                    f"[LEGACY] Failed to process connections for {input_name}: {e}"
                )
                connected[input_name] = None
    ...
    def process_processor_result(
        self, result: Any, state: FlowState, node_id: str
    ) -> Any:
        """
    ...
    def _execute_async_method(
        ...
        try:
            try:
                ...
                # Use keyword arguments for better compatibility
                result = asyncio.run(
                    execute_method(inputs=user_inputs, connected_nodes=connected_nodes)
                )
    ...
    def _execute_async_method_terminator(
        self,
        execute_method,
        previous_node: Any,
        user_inputs: Dict[str, Any],
        ...
        try:
            try:
                ...
                if loop.is_running():
                    ...
                    result = asyncio.run(
                        execute_method(previous_node=previous_node, inputs=user_inputs)
                    )
            except RuntimeError:
                # No event loop, create one
                result = asyncio.run(
            ...
            logger.error(
                f"Failed to execute async terminator method for {node_id}: {e}"
            )
            raise
    ...
    def _process_multiple_connections(
        self, connection_data: List[Dict[str, Any]], state: FlowState
    ) -> Any:
        """
    ...
    def _process_single_connection(
        self, connection_info: Dict[str, Any], state: FlowState
    ) -> Any:
        """
        ...
        try:
            ...
            if isinstance(connection_info, list):
                logger.error(
                    f"[ERROR] Failed to extract connection: list indices must be integers or slices, not str"
                )
                logger.error(
                    f"[ERROR] Expected dict but got list with {len(connection_info)} items: {connection_info}"
                )
                ...
                if len(conn_list) == 1 and isinstance(conn_list[0], dict):
                    ...
                    logger.error(
                        "[ERROR] Cannot process multi-item list in single connection context"
                    )
                    return None
            ...
            if not isinstance(connection_info, dict):
                logger.error(
                    f"[ERROR] Invalid connection format: {type(connection_info)}"
                )
                return None
    ...
    def _aggregate_connection_data(
        self, results: List[Any], connection_data: List[Dict[str, Any]]
    ) -> Any:
        """
        ...
        if connection_data and len(connection_data) == len(results):
            ...
            if any(priority > 0 for priority, _ in prioritized):
                ...
                logger.debug(
                    f"Priority selection: chose result with priority {prioritized[0][0]}"
                )
    ...
    def _validate_and_fix_connection_formats(
        self, connected: Dict[str, Any], node_id: str
    ) -> Dict[str, Any]:
        """
        ...
        for handle_name, connection_data in connected.items():
            try:
                ...
                if handle_name.lower() in ["tools", "tool", "tool_list"]:
                    validated_tools = self._validate_tools_connection(
                        connection_data, node_id, handle_name
                    )
                ...
                if isinstance(connection_data, list):
                    ...
                    if len(connection_data) == 1 and isinstance(
                        connection_data[0], dict
                    ):
                        logger.debug(
                            f"[FIX] Converting single-item list to dict for {node_id}.{handle_name}"
                        )
                        ...
                        logger.debug(
                            f"[KEEP] List format with {len(connection_data)} items for {node_id}.{handle_name}"
                        )
                    ...
                    validated[handle_name] = connection_data
                    logger.debug(
                        f"[KEEP] {type(connection_data)} format for {node_id}.{handle_name}"
                    )

            except Exception as e:
                logger.error(
                    f"[ERROR] Failed to validate connection format for {node_id}.{handle_name}: {e}"
                )
        ...
        if len(validated) != len(connected):
            logger.warning(
                f"[WARNING] Connection validation changed count from {len(connected)} to {len(validated)}"
            )

        return validated

    def _validate_tools_connection(
        self, connection_data: Any, node_id: str, handle_name: str
    ) -> Any:
        """
        ...
        try:
            if isinstance(connection_data, list):
                ...
                logger.debug(
                    f"[TOOLS] List format validated for {node_id}.{handle_name}: {len(connection_data)} items"
                )
                ...
                if "tools" in connection_data:
                    ...
                    logger.debug(
                        f"[TOOLS-FIX] Extracted tools from dict for {node_id}.{handle_name}: {type(tools_data)}"
                    )
                    return tools_data
                elif "tool" in connection_data:
                    tool_data = connection_data["tool"]
                    logger.debug(
                        f"[TOOLS-FIX] Extracted tool from dict for {node_id}.{handle_name}: {type(tool_data)}"
                    )
                    return tool_data
                else:
                    # Dict might be a tool object itself
                    logger.warning(
                        f"[TOOLS] Dict without 'tools' key for {node_id}.{handle_name}, keeping as-is"
                    )
                    return connection_data
            else:
                # Single tool object
                logger.debug(
                    f"[TOOLS] Single tool object for {node_id}.{handle_name}: {type(connection_data)}"
                )
                return connection_data

        except Exception as e:
            logger.error(
                f"[TOOLS-ERROR] Failed to validate tools connection for {node_id}.{handle_name}: {e}"
            )
            return connection_data

    def _extract_single_connection(
        self, input_name: str, connection_data: Any, state: FlowState
    ) -> Any:
        """
        ...
        try:
            logger.debug(
                f"[EXTRACT] Processing connection '{input_name}' with format: {type(connection_data)}"
            )

            # Handle multiple connections (list format) - CRITICAL FOR MANY-TO-MANY
            if isinstance(connection_data, list):
                logger.debug(
                    f"[EXTRACT] Multiple connections detected for '{input_name}': {len(connection_data)} items"
                )

                if not connection_data:
                    logger.warning(
                        f"[EXTRACT] Empty connection list for '{input_name}'"
                    )
                    return None
            ...
            # Handle single connection (dict format) - BACKWARD COMPATIBILITY
                ...
                logger.error(
                    f"[EXTRACT] Unsupported connection format for '{input_name}': {type(connection_data)}"
                )
                return None
