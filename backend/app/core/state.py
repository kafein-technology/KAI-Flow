...
class FlowState(BaseModel):
    ...
    memory_data: Dict[str, Any] = Field(
        default_factory=dict, description="General purpose memory storage"
    )

    # Last output from any node
    last_output: Optional[str] = Field(
        default=None, description="Output from the last executed node"
    )

    # Current input being processed
    current_input: Optional[str] = Field(
        default=None, description="Current input being processed"
    )

    # Node execution tracking
    executed_nodes: Annotated[List[str], merge_executed_nodes] = Field(
        default_factory=list, description="List of node IDs that have been executed"
    )
    ...
    session_id: Optional[str] = Field(
        default=None, description="Session identifier for persistence"
    )
    user_id: Optional[str] = Field(default=None, description="User identifier")
    owner_id: Optional[str] = Field(
        default=None,
        description="Owner identifier (e.g. workspace owner or workflow creator)",
    )
    ...
    variables: Dict[str, Any] = Field(
        ...
        description="Variables that can be set and accessed by nodes",
    )
    ...
    node_outputs: Annotated[Dict[str, Any], merge_node_outputs] = Field(
        default_factory=dict, description="Storage for individual node outputs"
    )
    ...
    def __init__(self, **data):
        ...
        if (
            not self.session_id
            or self.session_id == "None"
            or len(str(self.session_id).strip()) == 0
        ):
            import uuid

            self.session_id = f"state_session_{uuid.uuid4().hex[:8]}"
            logging.warning(
                f"[WARNING] No valid session_id in FlowState, generated: {self.session_id}"
            )
