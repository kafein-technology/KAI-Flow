...
class BaseWorkflowEngine(abc.ABC):
    ...
    @abc.abstractmethod
    def build(
        self, flow_data: JSONType, *, user_context: Optional[JSONType] = None
    ) -> None:
...
class StubWorkflowEngine(BaseWorkflowEngine):
    ...
    async def execute(
        ...
        *,
        ...
        if not self._BUILT:
            raise RuntimeError(
                "Workflow must be built before execution. Call build() first."
            )
            ...
            for i, node in enumerate(nodes):
                ...
                if node_type not in self._builder.node_registry:
                    ...
                    similar = [
                        ...
                        if (node_type or "").lower() in t.lower()
                    ]
                    if similar:
                        warnings.append(
